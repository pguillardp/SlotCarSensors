...
public class Scalex7042 extends Rs232 {
	...
	protected void parseFrame(byte[] from7042, int read) {
		...
		if (car >= 1 && car <= Scalex7042.MAX_CAR) {
			SensorPinImpl pin = this.getPin(SensorConstants.DETECTION + car);
	...
	public boolean setOutputPinValue(String pin, int value) {
		if (Scalex7042.GREEN_LED.equals(pin)) {
			this.setGreen(value > 0);
			return true;
		}
		if (Scalex7042.RED_LED.equals(pin)) {
			this.setRed(value > 0);
			return true;
		}
		if (SensorConstants.DIGITAL_LIGHT_PIN.equals(pin)) {
			for (int i = 0; i < MAX_CAR; i++) {
				if (value > 0) {
					this.onLED(i);
				} else {
	...
	protected void ioPinList() {
		...
		for (int i = 1; i <= MAX_CAR; i++) {
			...
			pin = new SensorPinImpl(this, "digital.out." + i, "light #" + i);
			pin.setBounds(191 + (i - 1) * 16, 100, 10, 10);
			this.pins.add(pin);
		}

		pin = new SensorPinImpl(this, "digital.out." + MAX_CAR + 1, Scalex7042.RED_LED);
		pin.setBounds(377, 91, 10, 10);
		this.pins.add(pin);

		pin = new SensorPinImpl(this, "digital.out." + MAX_CAR + 2, Scalex7042.GREEN_LED);
