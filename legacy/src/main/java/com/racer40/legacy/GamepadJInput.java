...
public class GamepadJInput /* extends SensorImpl */ {
	final Logger logger = LoggerFactory.getLogger(GamepadJInput.class);
	...
	// String identifier =
	// event.getComponent().getIdentifier().getName().toLowerCase().replace("button",
	// "")
	// .replace(" ", "");
	// SensorPinImpl pin = this.getPin("digital.in." + (Integer.parseInt(identifier)
	// + 1));
	// if (pin != null) {
	// pin.setPinValueForNotification(event.getValue() > 0 ? SensorConstants.PIN_ON
	...
	// return JInputManager.getInstance().getButtonState(this, index + "") ?
	// SensorConstants.PIN_ON
	// : SensorConstants.PIN_OFF;
	// }
	//
	// @Override
	...
	// p = new SensorPinImpl(this, "digital.in." + (Integer.parseInt(btn[0]) + 1),
	// btn[1]);
	// if (i < size2) {
